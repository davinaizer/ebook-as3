            ...
            if (EbookApi.getEbookModel().isConsultMode == false) {
                ...
                for (i = 0; i < listP.totalItems; i++) {
                    ...
                    if (firstPage && lastPage) {
                        ...
                        if (!firstPage)
                            throw new Error("IndexPanel.invalidateList >> firstPage ID not found: " + data.content[i].@pageID);
                        if (!lastPage)
                            throw new Error("IndexPanel.invalidateList >> lastPage ID not found: " + data.content[i].@lastPageID);
                    }
                }
            }
        }
